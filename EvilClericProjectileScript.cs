...
public class EvilClericProjectileScript : Projectile
{
    public override void OnTriggerEnter2D(Collider2D collider)
    {
        if (!gameObject.tag.Equals(collider.tag) && !collider.tag.Equals(gameObject.tag + "Base") && !collider.tag.Equals("background") && !gameObject.tag.Equals(collider.tag + "Base") && !collider.tag.Equals("treasureChest"))
        {
            ...
            if (randomNumber > 95)
            {
                collider.GetComponentInChildren<AllEffectsOnTroop>().AddToList(new CharmEffect(10, collider.transform.parent.gameObject));
            }
            else
            {
